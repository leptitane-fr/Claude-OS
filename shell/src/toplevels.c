...
shell_app_id_matches (const char *desktop_id, const char *app_id)
{
    ...
    /* Chromium s'annonce selon les versions « chromium » ou
     * « chromium-browser ». On accepte donc qu'un identifiant soit le prefixe
     * de l'autre, mais uniquement si la suite commence par un separateur :
     * sans cette condition, « file » et « filezilla » se confondraient. */
    const char *court = strlen (a) < strlen (b) ? a : b;
    const char *long_ = (court == a) ? b : a;
    size_t n = strlen (court);

    return strncmp (court, long_, n) == 0
        && (long_[n] == '-' || long_[n] == '_' || long_[n] == '.');
}
